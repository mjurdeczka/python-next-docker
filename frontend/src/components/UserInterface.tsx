...
const UserInterface = ({ backendName }: UserInterfaceProps) => {
  ...
  useEffect(() => {
    (async () => {
      try {
        const response = await fetchData(`${apiUrl}/api/${backendName}/users`, { method: "GET" });
  ...
  const createUser = async (e: React.FormEvent<HTMLFormElement>) => {
    e.preventDefault();
    try {
      const response = await fetchData(`${apiUrl}/api/${backendName}/users`, { body: JSON.stringify(newUser)});
  ...
  const handleUpdateUser = async (e: React.FormEvent<HTMLFormElement>) => {
    e.preventDefault();
    try {
      await fetchData(`${apiUrl}/api/${backendName}/users/${updateUser.id}`, {method: "PUT", body: JSON.stringify({ name: updateUser.name, email: updateUser.email })});
  ...
  const deleteUser = async (userId: number) => {
    try {
      await fetchData(`${apiUrl}/api/${backendName}/users/${userId}`, {method: "DELETE"});
